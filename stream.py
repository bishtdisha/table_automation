...
def add_styled_table_to_pdf(input_pdf_path, logo_img_path, sign_img_path, output_path, fields):
    try:
    ...
    for page in doc:
        ...
        for h in row_heights:
            page.draw_line((table_left_x, y), (table_left_x + table_width, y), color = sky_blue, width = 0.8)
            y += h
        ...
        page.draw_line((table_left_x + table_width, table_top_y), (table_left_x + table_width, table_bottom_y), color = sky_blue, width = 0.8)

        y = table_top_y
        for idx, (label, key) in enumerate(table_fields):
            h = row_heights[idx]
            ...
            if key == "submitted":
                ...
                name_rect = fitz.Rect(col_split_x + 3, y + 2, col_split_x + submitted_col_width - 50, y + h)
                page.insert_textbox(name_rect, submitted_name, fontsize = 7.8, fontname = "helv", color = text_color, align = 0)
                sign_target_height = h - 4
                if sign_img.height > 0:
                    scale = sign_target_height / sign_img.height
                    sign_target_width = sign_img.width * scale
                    sign_rect = fitz.Rect(col_split_x + submitted_col_width - sign_target_width - 4, y + 2,
                                          col_split_x + submitted_col_width - 4, y + 2 + sign_target_height)
        ...
        logo_rect = fitz.Rect(table_left_x + 2, logo_row_top, table_left_x + table_width - 2, table_bottom_y - 2)
...
if submitted_btn:
    if not all([pdf_file, logo_file, sign_file]):
        ...
        try:
            ...
            add_styled_table_to_pdf(input_pdf_path, logo_path, sign_path, output_pdf_path, field_data)
